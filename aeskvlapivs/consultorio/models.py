...
class Urgencias(models.Model):
    ...
    Est_doloroso = 2
    No_Apertura = 1 
    AP_OCULAR = [(Espontanea, int(4)), (Orden_verbal, int(3)), (Est_doloroso, int(2)), (No_Apertura, int(1))]
    apertura_ocular = models.PositiveSmallIntegerField(blank=True, null=True, default=1)
    ...
    RESP_VERBAL = [(Orientado, int(5)), (Desorientado_Confuso, int(4)), (Incoherente, int(3)), (Sonidos_incomprensibles, int(2)), (Ausencia_Respuesta, int(1)),]
    Respuesta_Verbal = models.PositiveSmallIntegerField(blank=True, null=True, default=1)
    ...
    Flexion_anormal = 3
    Extensión_anormal = 2
    Ausencia_de_respuesta  = 1
    RESP_MOTORA = [(Obedece_ordenes, int(6)), (Localiza_dolor, int(5)), (Retirada_al_dolor, int(4)), (Flexion_anormal, int(3)),
    (Extensión_anormal, int(2)), (Ausencia_de_respuesta, int(1)),]
    Respuesta_Motora = models.PositiveSmallIntegerField(blank=True, null=True, default=1)
    ...
    def consid(self):
        if self.ESCALA_DE_GLASGOW < 9:
            return 'Considere Manejo Avanzado de Vía Aérea'
